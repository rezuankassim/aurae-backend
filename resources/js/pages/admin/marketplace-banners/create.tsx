...
export default function CreateMarketplaceBanner() {
    const [isActive, setIsActive] = useState(true);

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Create Marketplace Banner" />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl px-4 py-6">
                <Heading title="Create Marketplace Banner" description="Upload a banner image for marketplace page" />

                <Form
                    {...MarketplaceBannerController.store.form()}
                    options={{
                        preserveScroll: true,
                    }}
                    resetOnSuccess
